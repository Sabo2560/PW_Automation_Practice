...
6. **[Confirmed via a real CI failure, testing-environment quirk, not a bug]** Whether WebKit moves focus onto the 'Open Modal' trigger button when it's clicked (and therefore whether MUI's close-time focus restoration lands there again) is OS-dependent, not merely engine-dependent: confirmed live on local Windows Playwright WebKit that the button never becomes `document.activeElement` on click; confirmed deterministically on CI's Linux WebKit build (GitHub Actions, `ubuntu-24.04`), across the initial attempt and both configured retries, that clicking the button DOES focus it — the opposite outcome from the identical test code. `WindowPage.expectClosedStateFullyRestored()` therefore skips the focus-restoration assertion entirely on WebKit (asserting neither state) rather than asserting a specific expectation, since neither one is portable across the environments this suite actually runs in.
